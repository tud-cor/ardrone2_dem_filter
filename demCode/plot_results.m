function plot_results(output,model,brain,if_UIO,if_cause,xh)
%% Plot the results
...
if if_cause == 1
    ...
    subplot(2,1,1);
    fig1 = plot(output.DEM_t,model.ideal_x(:,xObs),'Color',color(1,:));
    hold on;
    fig2 = plot(output.DEM_t,output.DEMv_x(:,xObs),'--','Color',color(2,:));
    fig3 = plot(output.DEM_t,output.kalmfv_x(xObs,:),'-.','Color',color(3,:));
    ...
    subplot(2,1,2);
    fig1 = plot(output.DEM_t,model.ideal_x(:,xh),'Color',color(1,:));
    hold on;
    fig2 = plot(output.DEM_t,output.DEMv_x(:,xh),'--','Color',color(2,:));
    fig3 = plot(output.DEM_t,output.kalmfv_x(xh,:),'-.','Color',color(3,:));
    legend([fig1(1),fig2(1),fig3(1)],...
           {'State reference','DEM hidden state estimate','KF hidden state estimate'});
    xlabel('Time (s)');
    ylabel('State amplitude (-)');
else
    ...
    subplot(3,1,1);
    fig1 = plot(model.t,model.ideal_x(:,xObs),'Color',color(1,:));
    hold on;
    fig2 = plot(output.DEM_t,output.DEM_x(:,xObs),'--','Color',color(2,:));
    fig3 = plot(model.t,output.kalman_x(xObs,:)','-.','Color',color(3,:));
%     if if_UIO == 1
...
%     end
    ...
    subplot(3,1,2);
    fig1 = plot(model.t,model.ideal_x(:,xh),'Color',color(1,:));
    hold on;
    fig2 = plot(output.DEM_t,output.DEM_x(:,xh),'--','Color',color(2,:));
    fig3 = plot(model.t,output.kalman_x(xh,:)','-.','Color',color(3,:));
%     if if_UIO == 1
%         fig4 = plot(model.t,output.UIO_x_est,'-.','Color',color(4,:));
%         legend([fig1(1),fig2(1),fig3(1),fig4(1)],{'Ideal states (outputs with noise)','DEM states',...
%             'Kalman estimate','UIO estimate'});
...
%     end
    ...
    hold on;
    plot(output.DEM_t,output.DEM_x(:,brain.nx*(brain.p+1)+1),'--');
%     if if_UIO == 1
