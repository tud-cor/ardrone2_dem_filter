%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% D-step of DEM with unknown input
%
% Function to obtain the filtered state and input estimates using the
% D-step of DEM.
% 
...
% Create matrix coefficients for filter implementation
...
% Create filter Jacobian (eq. 37 in DEM paper)
Vuu = -[k1 k2; k3 k4]*blkdiag(V0,W0)*[k1 k2; k3 k4]';
Vuy = [k1 k2; k3 k4]*blkdiag(V0,W0)*...
                [kron(eye(p+1),eye(ny)); ...
                 zeros(nx*(p+1)+nv*(d+1),ny*(p+1))];
Vuv = [k1 k2; k3 k4]*blkdiag(V0,W0)*...
                [zeros(ny*(p+1),nv*(d+1)); -eye(nv*(d+1)); ...
                 zeros(size(W0,1),nv*(d+1))];

dfdu = [Vuu Vuy Vuv; ...
        zeros(size(Dy,1),size([Vuu Vuy Vuv],2));...
        zeros(size(Dv,1),size([Vuu Vuy Vuv],2))]  + blkdiag(Da,Dv,Dy,Dv);

% Obtain filter results using Newton-Gauss method (eq. 38 in DEM paper)
for i = 2:nt
    if if_predict_y
        q = [DEM_xx(i-1,1:nx*(p+1)+nv*(d+1)) ...
             Y_embed(1:ny*(p+1)+nv*(d+1),i-1)'];
        f = blkdiag([A1; A2],A3,A4)*q' + [B1;B2;B3;B4]*Y_embed(:,i-1);
        DEM_xx(i,:) = q + ((expm(dfdu*sam_time)-eye(size(f,1)))*...
                           pinv(dfdu)*f)';
    else
