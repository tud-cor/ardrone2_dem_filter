...
% Result plotting
%
% Function to plot the observer state and input estimates results.
% 
...
if if_cause
    ...
    hold on;
    fig2 = plot(output.DEM_t,output.DEMv_x(:,1:brain.nx),'--',...
                'Color',color(2,:));
...
% Plot state estimates with unknown input
    ...
    hold on;
    fig2 = plot(output.DEM_t,output.DEM_x(:,1:brain.nx),'--',...
                'Color',color(2,:));
    fig3 = plot(model.t,output.kalman_x','-.','Color',color(3,:));
%     if if_UIO == 1
%         fig4 = plot(model.t,output.UIO_x_est,'-.','Color',color(4,:));
%         legend([fig1(1),fig2(1),fig3(1),fig4(1)],...
%                {'Ideal states (outputs with noise)','DEM states',...
...
%     else
    ...
    subplot(2,1,2);
    fig1 = plot(model.t,model.real_cause','Color',color(1,:),...
                'LineWidth',1);
    hold on;
    fig2 = plot(output.DEM_t,...
                output.DEM_x(:,brain.nx*(brain.p+1)+1:...
                               brain.nx*(brain.p+1)+brain.nv),...
                '--','Color',color(2,:));
%     if if_UIO == 1
%         plot(model.t,output.UIO_v_est,'-.');
%         legend('Ideal causal state','DEM cause estimate',...
%                'UIO cause estimate');
%     else
